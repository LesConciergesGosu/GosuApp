...
+ (NSString *)imageCacheDirectory
{
    ...
    NSString *path = [[DataManager cacheDirectory] stringByAppendingPathComponent:@"images"];
...
- (void) loadImageBigURLRequest:(NSURLRequest *)request handler:(void (^)(UIImage *image))handler {
    ...
    NSString *lastName = [[[request URL] absoluteString] lastPathComponent];
    NSString *path = [[DataManager imageCacheDirectory] stringByAppendingPathComponent:lastName];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [UIImageJPEGRepresentation(responseObject, 1) writeToFile:path atomically:YES];
