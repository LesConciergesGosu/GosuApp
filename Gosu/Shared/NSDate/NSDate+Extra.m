...
//  SimplePhotoBook
//
//  Created by Dragon on 9/25/14.
//  Copyright (c) 2014 Picaboo. All rights reserved.
//

#import "NSDate+Extra.h"


static const NSUInteger componentFlags = (NSYearCalendarUnit| NSMonthCalendarUnit | NSDayCalendarUnit | NSWeekCalendarUnit |  NSHourCalendarUnit | NSMinuteCalendarUnit | NSSecondCalendarUnit | NSWeekdayCalendarUnit | NSWeekdayOrdinalCalendarUnit);
...
- (NSDate *) dateInOneHour
{
    NSDate *date = [self dateByAddingHours:1];
...
+ (NSDate *) dateAtStartOfYear:(NSInteger)year
{
    NSDateComponents *dateComponents = [[NSDateComponents alloc] init];
...
+ (NSDate *) dateAtEndOfYear:(NSInteger)year
{
    NSDateComponents *dateComponents = [[NSDateComponents alloc] init];
    [dateComponents setYear:year];
...
+ (NSString *)timeStringFromTimeInterval:(NSTimeInterval)interval
{
    ...
    time = time > 24 * 3600 ? time - 24 * 3600 : time;
    
    BOOL am = time < 12 * 3600;
    if (am)
    {
        long hour = time / 3600;
