    ...
    if (user.photo) {
        ...
        [[DataManager manager] loadImageURLRequest:request handler:^(UIImage *image) {
            ...
            if (image) {
                ...
                [sself.photoImageView setImage:[UIImage imageNamed:@"buddy"]];
            }
        }];
        
    } else {
        [self.photoImageView setImage:[UIImage imageNamed:@"buddy"]];
    }
}
...
- (void)buttonCell:(ButtonCell *)cell tappedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (indexPath.section == ProfileSectionInformation) {
    ...
    } else if (indexPath.section == ProfileSectionPayment) {
    ...
    } else if (indexPath.section == ProfileSectionFamilyFriends) {
    ...
    switch (indexPath.section) {
        ...
        case ProfileSectionPayment:
        {
            if (indexPath.row < [self.cards count]) {
                ...
                NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:text attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14], NSForegroundColorAttributeName:APP_COLOR_TEXT_GRAY}];
                [str setAttributes:@{NSForegroundColorAttributeName:APP_COLOR_GREEN} range:[text rangeOfString:title]];
