...
- (void) groupBubbleDataInBackgroundWithSkip:(NSInteger)skip
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        for (NSInteger i = skip; i < [messages count] + 1; i++)
        {
            ...
            if (i == 0) // task description
            {
                ...
                bubble.type = self.task.voice != nil ? MessageTypeDescription : MessageTypeText;
                bubble.isMe = self.task.customer == me;
            }
            else
            {
                author = [(Message *) messages[i - 1] author];
                bubble = [BubbleMessage bubbleWithMessage:messages[i - 1]];
            ...
            switch (bubble.type) {
                ...
                {
                    CGSize textSz = [bubble.text sizeWithFont:font constrainedToSize:CGSizeMake(BUBBLE_MAX_WIDTH - 30, 1000) lineBreakMode:NSLineBreakByWordWrapping];
